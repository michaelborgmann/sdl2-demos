...
void Image::loadImage(std::string filename) {
    //std::tr1::shared_ptr<SDL_Surface> bitmap(SDL_LoadBMP(filename.c_str()),
    std::shared_ptr<SDL_Surface> bitmap(SDL_LoadBMP(filename.c_str()),
                                        SDL_FreeSurface);
    if (!bitmap) { throw Error(SDL_GetError()); }

    //texture = std::tr1::shared_ptr<SDL_Texture> (SDL_CreateTextureFromSurface(renderer, bitmap.get()),
